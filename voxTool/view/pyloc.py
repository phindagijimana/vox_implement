...
if __name__ == '__main__':
    _cfg = os.path.join(os.path.dirname(__file__), '..', 'config.yml')
    with open(_cfg, encoding='utf-8') as _f:
        _config = yaml.safe_load(_f)
    controller = PylocControl(_config)
...
if __name__ == 'x__main__':
    ...
    _mcfg = os.path.join(os.path.dirname(__file__), '..', 'model', 'config.yml')
    with open(_mcfg, encoding='utf-8') as _mf:
        _mconf = yaml.safe_load(_mf)
    x = LeadDefinitionWidget(None, _mconf)
    x.show()
    window = QtGui.QMainWindow()
